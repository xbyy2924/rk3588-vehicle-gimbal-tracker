...
/* M2 motor/encoder calibration verified by M2_ECAL2 and M2_CLID1. */
#define MOTOR_POLE_PAIRS                         7
#define ENCODER_DIRECTION                       -1
#define MECHANICAL_ZERO_RAW                      0
#define ELECTRICAL_OFFSET_DEG                26.558f
...
/* DUAL_CASCADE2 M2: higher stiffness/bandwidth with matched damping. */
#define POSITION_GAIN_PER_S                       15.0f
#define SPEED_REFERENCE_LIMIT_DPS               220.0f
#define SPEED_BANDWIDTH_HZ                         8.0f
#define SPEED_DAMPING_RATIO                        1.60f
...
/* Same estimator as M1; only the identified M2 initial range is independent. */
#define INVERSE_PLANT_INITIAL_A_PER_RAD_S2      0.00058f
#define INVERSE_PLANT_MINIMUM_A_PER_RAD_S2      0.00025f
#define INVERSE_PLANT_MAXIMUM_A_PER_RAD_S2      0.00080f
...
static Motor2CalibrationData s_data;
...
bool Motor2Calibration_Init(void)
{
    ...
    s_data.servo_state = M2_FOC_SERVO_IDLE;
    ...
    if (HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_3) != HAL_OK) return false;
    pwm_neutral();
...
void Motor2Calibration_Task(void)
{
    ...
    AS5600_GetDataAxis(AS5600_MOTOR2, &encoder);
    PhaseCurrent_GetData(&current);

    if (!s_data.initialized || s_data.servo_state == M2_FOC_SERVO_FAULT)
        return;

    if (!encoder_valid(&encoder))
    {
        if (s_data.servo_state != M2_FOC_SERVO_IDLE)
            Motor2Calibration_Stop(M2_FOC_FAULT_ENCODER);
        return;
    }
    if (!current.calibrated || !current.m2_offset_valid) return;
    if (HAL_GPIO_ReadPin(M2_nFAULT_GPIO_Port,
                         M2_nFAULT_Pin) == GPIO_PIN_RESET)
    {
        Motor2Calibration_Stop(M2_FOC_FAULT_NFAULT);
        return;
    }
    if (current.m2_ia_raw <= ADC_RAIL_LOW_COUNT ||
        current.m2_ia_raw >= ADC_RAIL_HIGH_COUNT ||
        current.m2_ib_raw <= ADC_RAIL_LOW_COUNT ||
        current.m2_ib_raw >= ADC_RAIL_HIGH_COUNT)
    {
        Motor2Calibration_Stop(M2_FOC_FAULT_ADC_RAIL);
        return;
    }
    ...
    if (s_data.servo_state == M2_FOC_SERVO_IDLE)
    {
        s_data.encoder_raw = encoder.raw_angle;
        s_data.encoder_total_count = encoder.total_count;
        begin_state(M2_FOC_SERVO_USB_WAIT);
        return;
    }

    const uint32_t now = HAL_GetTick();
    s_data.state_elapsed_ms = now - s_state_start_ms;
    if (s_data.servo_state == M2_FOC_SERVO_ACTIVE)
        s_data.total_elapsed_ms = now - s_servo_start_ms;

    if (s_data.servo_state == M2_FOC_SERVO_USB_WAIT)
    {
        s_data.encoder_raw = encoder.raw_angle;
        s_data.encoder_total_count = encoder.total_count;
        ...
        begin_state(M2_FOC_SERVO_ACTIVE);
        return;
    }
...
void Motor2Calibration_FastLoop(float ia, float ib, float ic)
{
    ...
    {
        Motor2Calibration_Stop(M2_FOC_FAULT_OVERCURRENT);
        return;
    }
...
{
    if (fault == M2_FOC_FAULT_NONE) return;
    s_data.fault = fault;
    s_data.servo_state = M2_FOC_SERVO_FAULT;
    stop_output();
}

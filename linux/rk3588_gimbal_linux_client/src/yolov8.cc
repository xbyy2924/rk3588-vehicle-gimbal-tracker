...
{
    if (model_path == NULL || app_ctx == NULL) return -1;
    int ret;
    int model_len = 0;
    char *model = NULL;
    rknn_context ctx = 0;
    ...
    if (ret != RKNN_SUCC)
    {
        fprintf(stderr, "rknn_query input/output count failed: ret=%d\n", ret);
        rknn_destroy(ctx);
        return -1;
    }
    if (io_num.n_input == 0 || io_num.n_output == 0) {
        fprintf(stderr, "invalid model I/O count: input=%u output=%u\n",
    ...
    memset(input_attrs.data(), 0, input_attrs.size() * sizeof(input_attrs[0]));
    for (int i = 0; i < io_num.n_input; i++)
    {
        ...
        if (ret != RKNN_SUCC)
        {
            fprintf(stderr, "rknn_query input attr failed: ret=%d\n", ret);
            rknn_destroy(ctx);
            return -1;
        }
    ...
    memset(output_attrs.data(), 0, output_attrs.size() * sizeof(output_attrs[0]));
    for (int i = 0; i < io_num.n_output; i++)
    {
        ...
        if (ret != RKNN_SUCC)
        {
            fprintf(stderr, "rknn_query output attr failed: ret=%d\n", ret);
            rknn_destroy(ctx);
            return -1;
        }
    ...
    if (app_ctx->input_attrs == NULL || app_ctx->output_attrs == NULL) {
        fprintf(stderr, "allocate RKNN tensor attributes failed\n");
...
{
    if ((!app_ctx) || !(img) || (!od_results) || app_ctx->io_num.n_input == 0 ||
    ...
    memset(inputs.data(), 0, inputs.size() * sizeof(inputs[0]));
    ...
    {
        fprintf(stderr, "malloc inference buffer failed: size=%d\n", dst_img.size);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fprintf(stderr, "convert_image_with_letterbox failed: ret=%d\n", ret);
        goto out;
    }
    ...
    // Post Process
    ret = post_process(app_ctx, outputs.data(), &letter_box,

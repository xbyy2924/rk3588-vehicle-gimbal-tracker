...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        int id = classId[n];
        // indexArray maps the sorted/NMS result back to the original vectors.
        // Using i here assigns another detection's confidence and can prevent
        // the tracker from accepting an otherwise valid vehicle box.
...
{
    if (label_path == nullptr || label_path[0] == '\0') return -1;
    const int ret = loadLabelName(label_path, labels);
    if (ret < 0)
    {
        fprintf(stderr, "Load labels failed: %s\n", label_path);
        deinit_post_process();
        return -1;
    }
    return 0;
